...
%w(base_model client contact project task user rate_limit_status task_assignment user_assignment expense_category invoice expense time_entry).each {|a| require "harvest/#{a}"}
%w(base account clients contacts projects tasks users task_assignments user_assignments expense_categories invoices expenses time reports).each {|a| require "harvest/api/#{a}"}
